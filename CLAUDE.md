...
- Monitor specs are `.monitor.json`. Monitors can declare `classify.agent` referencing a `.agent.yaml` spec — the classify call then uses the agent spec compilation pipeline (`compileAgentSpec()`, `createAgentLoader()`) with JSON output validated against `verdict.schema.json`. This gives monitors the same typed contracts as workflow agent steps (model, thinking, output.schema). Collectors populate the template context (same role as `contextBlocks` for workflow agents). Monitors without `classify.agent` use the legacy text-parsing path. Template search: project `.pi/monitors/` > user `~/.pi/agent/monitors/` > package `examples/`.
