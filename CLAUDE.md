...
While the ready/blocked deriver does not honor gating relations (`FGAP-061`) and there is no release vocabulary (`FGAP-011`), **current status + focus and the required-work ordering are NOT derivable from `/context status` — this note is the temporary record.**
...
1. **FGAP-061** (fold `*_gated_by_item` into `currentState`) + **FEAT-004** (config-declared, all-kinds readiness) → focus/status/blocked become derivable; **retires this note**.
