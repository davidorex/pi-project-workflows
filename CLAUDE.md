...
While the ready/blocked deriver does not honor gating relations (`FGAP-061`), the **current status + focus** is **not** derivable from `/context status`. **FEAT-006 (`pi-context update`) and FEAT-008 (best-of-breed `pi-context-cli` surface) are COMPLETE** (FEAT-008: TASK-015..019 + 040/042, VER-029..035; FGAP-021..026/032/062/063/064/072 closed). **The User Stories block is LIVE** (existing `story` kind relabeled; `user_kind` field; `task_advances_story`/`feature_advances_story` + the `story-advancers`/`story-advancers-features` lenses; STORY-001..014 backfilled verbatim; enforcement of "complete requires every advanced story met" arrives with TASK-047/FGAP-082, the `advancer-completion` invariant class). The current focus: **FEAT-009** (update blocked-diagnostic + resolution loop, story-driven; approved) — **TASK-046 + TASK-048 + TASK-051 COMPLETE** (VER-038/039/040; FGAP-066/077/080 closed; stories 001–006, 008, 009, 011, 012 met — `--dryRun` predicts the precise outcome; blocked carries `blockedDetail` + persists a pending-blocked record; `validate-block-items` returns per-item failures; the authGated `resolve-blocked` op commits a resolution and converges). **TASK-052 COMPLETE** (VER-041; FGAP-081 closed; STORY-013/014 met — in-file failure markers are the DEFAULT blocked behavior, byte-exact recoverable, stripped by `resolve-blocked`; the invalid opt-in implementation is preserved on `feat/task-052-markers-fail` pending user-directed deletion). **TASK-049 COMPLETE** (VER-043; FGAP-030 + FGAP-078 closed; STORY-007 met — the `context-check-status` op reports per schema the drift state, versions, and for behind schemas the delta or content-only basis). **TASK-050 (catalog diffable, FGAP-079) is the LAST FEAT-009 slice**; 13 of 14 stories met. TASK-053 complete (VER-042, the registry-seeding helper + sentinel constant); TASK-054 filed (the raw-write helper). Adjacent new: FGAP-086 ($id-keyed validator cache), FGAP-087 (non-array block content resyncs unvalidated — raw). **Next: TASK-050.** Adjacent open: TASK-047 (invariant class); DEC-0018 (open, awaiting enactment); FGAP-076 + issue-003; FGAP-083/084/085 (CLI frictions, raw/investigated); TASK-041; TASK-044; TASK-033 (FEAT-007); FGAP-065. Delete this note when `FGAP-061` lands (gate-aware derivation).
