...
**Surfaced, unfiled (awaiting scope) — catalog↔config relation_type drift:** the packaged catalog (`samples/conception.json`, 34 relation_types) is a strict subset of live `.context/config.json` (40); 6 live-only — `task_gated_by_item` (the relation TASK-065's deriver consumes), `session_touches_item`, `decision_derived_from_item`, `decision_escalates_underdetermined`, `task_advances_story`, `feature_advances_story` — so a fresh `accept-all` substrate lacks them and the write-time validator rejects their edges (unwritable, not merely inert). No forcing-function ensures catalog ⊇ shipped-derivation vocabulary. Class-level gap, `task_gated_by_item` the trigger; investigation `analysis/2026-06-16-task-gated-by-item-catalog-config-drift.md`; relate FGAP-060/FGAP-010, cite FGAP-090's incomplete back-port + FGAP-061 as exposed consumer.
