...
**Filing provenance (the `filing-provenance` convention, binding):** every semantic element of a planning-block filing carries one of three provenances — user-VERBATIM, user-DIRECTED, or DERIVABLE from a cited fact/convention/decision; anything else is augmentation and does not go in. A qualifier that narrows what the user said (a mode, an opt-in, a flag, a tier, a deferral) is never derivable — it is a cited user decision or absent. An item bound to user stories is DIFFED against the stories' verbatim statements at filing, and every explore/plan/probe brief for a story-bound item includes that verbatim-delta check ("any delta is a filing defect to surface, not a requirement to implement"). The planning-block write guard gates every such write on the `# provenance-reviewed` sentinel; the sentinel attests the element-by-element review happened. Filings are composed verbatim downstream — an augmentation at filing becomes the requirement everywhere it is consumed.
