...
- Husky pre-commit runs `npm run check`, `npm test`, then the gate scripts: `check-changelog` (published-surface commits must grow `[Unreleased]`), `parity-check` (op↔CLI parity), `check-config-schema` (non-additive config.schema.json diffs need a version advance + packaged config migration — expand-contract), and `check-context-currency` (blocks the commit while the active substrate's DERIVABLE context is not current — any `anchor-drift`/`staleness-candidate` on a LIVE item, or any pending reconcile stored-status delta / staleness transition; terminal/settled items are exempt since their pins are historical by definition; fail-open on an absent/broken substrate). Never `--no-verify`. Fix root cause + new commit on hook failure.
