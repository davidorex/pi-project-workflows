...
While the ready/blocked deriver honors only task-level gating (`task_gated_by_item`, shipped via TASK-065; feature/story gates + config-declared all-kinds readiness remain `FGAP-061` FORWARD → FEAT-004) and there is no release vocabulary (`FGAP-011`), **current status + focus and the required-work ordering are NOT fully derivable from `/context status` — this note is the temporary record.**

**Done this stretch — FGAP-090 arc (committed on `context-jit-spec-v2`; `main` fast-forwarded earlier, now behind):** **FGAP-090 CLOSED** — relation_type registry coverage + write-time edge enforcement: added relation_types `decision_raises_gap`/`decision_gated_by_item`/`gap_relates_to_gap` (config + catalog, TASK-061); the write-time edge kind/registration validator hoisting the `validateContext` check to `appendRelationByRef`/`appendRelationsByRef`, distinct from TASK-027 (TASK-062); edge corrections — DEC-0018→FGAP-076 retyped to `decision_raises_gap`, `decision_gated_by_item` + `gap_relates_to_gap` edges added, FGAP-092 filed+cited as DEC-0018's true forcing artifact (TASK-063); VER-047/048/049. **DEC-0018 ENACTED** (was mis-statused open; per-component update model, in code by construction). **DEC-0013/0014/0015 enacted.** **FGAP-091 filed** (no forcing-function for warranted non-invariant closure-table edges) + **TASK-064** backfilled the 7 warranted task→decision/gap edges (TASK-010/011→DEC-0008/FGAP-012, TASK-012→DEC-0009/FGAP-015, TASK-014→FGAP-014); FGAP-091's forcing-function invariant is its open residual. **FGAP-093 filed** (write-time edge guard lacks full write↔validate parity — dangling/unregistered-endpoint + cycle accepted at write).

**Done this stretch — FGAP-061 NOW slice (committed on `context-jit-spec-v2`):** **TASK-065** extended `currentState` to fold `task_gated_by_item` gates into task readiness — a task gated by a non-`complete`-bucket item is reported blocked (gate id in `blockedBy`, unioned with dep parents) and held out of `nextActions`; gate satisfaction reuses `bucket(target)==="complete"` (kind-general, the status-consistency mechanism, not net-new); dangling gate = satisfied; scope is the literal `task_gated_by_item` (decision/feature/story gates inert). Verified: gates + real-CLI runtime demo (blocked→ready on gate close) + fresh adversarial probe (11 criteria, incl. the `blockersOf` dedup criterion appended post-probe per the user); VER-051; merged (`3872012`). **FGAP-061 stays `identified`** — only the NOW slice landed; FORWARD (feature/story readiness + config-declared blocked-by set) is FEAT-004, already edged `gap_addressed_by_feature → FEAT-004`.

**Filed — FGAP-094 (catalog↔config relation_type drift, class-level, P2, identified):** the packaged catalog (`samples/conception.json`, 34 relation_types) is a strict subset of live `.context/config.json` (40); 6 live-only — `task_gated_by_item` (the relation TASK-065's deriver consumes), `session_touches_item`, `decision_derived_from_item`, `decision_escalates_underdetermined`, `task_advances_story`, `feature_advances_story` — so a fresh `accept-all` substrate lacks them and the write-time validator rejects their edges (unwritable, not merely inert). No forcing-function ensures catalog ⊇ shipped-derivation vocabulary. Resolution: NOW back-port the catalog-worthy subset; FORWARD a build-time catalog⊇consumed-vocabulary parity check (FGAP-010 precedent). Edges `gap_relates_to_gap` → FGAP-060/FGAP-010/FGAP-061; investigation `analysis/2026-06-16-task-gated-by-item-catalog-config-drift.md`. Unscheduled (no task yet).

**Open backlog — 45 open gaps, coverage sparse (`analysis/2026-06-14-gap-task-feature-coverage-audit.md`): only ~6 actionably task-covered, 5 tasks under features, 4 release-pinnable features (FEAT-001..004) cover 6/45 gaps; orphan clusters FGAP-052..059 (convention debt), FGAP-033..045 (schema-model).** Required-work ordering to make focus/status derivable + releases pinnable:
1. **FGAP-061 NOW slice DONE** (task-level `task_gated_by_item` folded into `currentState`, TASK-065); **FEAT-004** (config-declared, all-kinds readiness incl. feature/story gates) is the remaining FORWARD residual → then focus/status/blocked fully derivable + **this note retires**.
...
**Releases HELD** (no `release:*` without explicit per-release authorization); v0.31.0 last; npm publish + git push are the user's downstream steps (OTP). Delete this note when `FEAT-004` lands (config-declared all-kinds gate-aware derivation); `FGAP-061`'s NOW slice already shipped via TASK-065.
