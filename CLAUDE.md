...
While the ready/blocked deriver does not honor gating relations (`FGAP-061`), the **current status + focus** is **not** derivable from `/context status`. **FEAT-006 (`pi-context update`) and FEAT-008 (best-of-breed `pi-context-cli` surface) are COMPLETE** (FEAT-008: TASK-015..019 + 040/042, VER-029..035; FGAP-021..026/032/062/063/064/072 closed). The current focus: **the user-stories block** — the held proposal `analysis/2026-06-10-user-stories-block-DRAFT.md` (existing `story` kind as "User Stories"; `user_kind` field; `task_advances_story`/`feature_advances_story`; completion blocked until every advanced story is met), with the 14 operator stories in `analysis/2026-06-10-update-blocked-diagnostic-task-DRAFT.md` as the first population — awaiting user approval to implement. Adjacent open work: the `update` arc — TASK-046 (faithful `--dryRun`, FGAP-066), DEC-0018 (per-component update model, open, awaiting enactment), FGAP-076..081 + issue-003 (blocked diagnostic/legibility/resolution-loop set, grounded by R-0012); TASK-041 (decision-derivation backfill); TASK-044 (broaden the register guard); TASK-033 (ship governance vocab to the catalog, FEAT-007); FGAP-065 (block_kind-materialization follow-up to FEAT-006). Delete this note when `FGAP-061` lands (gate-aware derivation).
