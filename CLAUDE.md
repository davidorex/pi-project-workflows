...
3. **Iterate to zero — in terms of the SUCCESS CRITERIA.** The task's filed `acceptance_criteria` are the contract and the terminating condition: when every criterion is verified true and the deterministic gate is green, iterate-to-zero is satisfied — even if the audit's total findings are nonzero. **No infinite adversary:** an audit TESTS the criteria; it does not define them. Every finding maps to a criterion id — a finding that FALSIFIES a criterion re-enters this same pipeline through explore → plan → approve → agent, scoped to the finding's whole **class** (Explore enumerates the class; fix the class, not the one symptom); a finding that falsifies NO criterion is FILED (per Experience-Gap Handling / explore-before-file) and is not fixed in this task. Filed findings stay owed — filing is not burial, so criteria-anchored termination is not scope reduction.
4. **Re-verify the fix — fresh, non-inherited.** A fix to a criterion-falsifying finding does NOT inherit the prior green: re-run the runtime demo + a FRESH adversarial re-audit of the fix against the falsified criteria. Loop 2→3→4 until every acceptance criterion is verified true.
5. **Docs — check + sync the surface (the `docs-surface-sync` convention in `.context`, binding).** For any user-facing surface change: CHECK the package + monorepo READMEs for statements the change makes stale and correct them (audit, not only append); update the surfaced strings of any changed reflected op (`description`/`promptSnippet` in `ops-registry.ts`); update `skill-narrative.md` when the surface is a pi op; regen SKILL.md (`npm run skills`) when a skill source or op string changed. Usage-only — fix/defect/correction framing → CHANGELOG `[Unreleased]`, never docs. **Brief-level enforcement:** every implementation brief for a surface change NAMES this audit explicitly (monorepo + relevant package README(s) + op strings + SKILL enumerated as the agent's deliverable); a brief without it is incomplete; a probe finding of a stale statement on a surface the brief names falsifies that deliverable and loops, and a stale-text finding on a surface neither the task's criteria nor the brief names is FILED, not fixed in this task.
...
**Autonomous-loop authorization (the `mandate-008` exception for the deterministic loop, binding).** `mandate-008` — a subagent returns an issue → STOP, report, the user decides — governs when the continue/stop signal is an LLM judgement (a blocker, an ambiguity, a scope/value/authority call). It does NOT stop the iterate-to-zero loop of steps 2→3→4. Autonomous re-dispatch of a FRESH agent (a fix agent, then a fresh adversarial re-audit) IS permitted and REQUIRED iff the loop terminates on a DETERMINISTIC gate — `npm run build` + `npm run check` + `npm test` exit 0 and the runtime-demo real-check, never an LLM read of "looks clean" — AND a SEPARATE fresh adversarial audit (not the implementer) reports every acceptance criterion CONFIRMED; it is bounded by a hard iteration cap, on hitting which the orchestrator STOPS and escalates per `mandate-008`. `done ≡ (deterministic gate exit 0) AND (every success criterion CONFIRMED)` — even if the audit's total findings are nonzero. **Audit posture:** the audit brief carries the task's filed `acceptance_criteria` verbatim; the auditor reports each criterion by id as CONFIRMED or a finding, never grades severity, never recommends; the orchestrator computes GO/NO-GO per criterion (any criterion false → NO-GO), and the per-criterion verdicts are filed as the verification item the turn the audit returns. Inside the loop the orchestrator ends each turn by dispatching the next agent (fix → re-audit → fix …) — its message terminates in the next tool call, never in a report that yields control — and surfaces to the user ONLY at a genuine LLM-judgement fork (a scope/value/authority decision, e.g. a HELD merge or release per the release-hold) or the iteration cap. A criterion-falsifying finding from the audit/probe is work the loop closes autonomously, scoped to the finding's whole class, not an issue handed to the user; handing a policy-determined criterion-falsifying fix to the user instead of looping it is the mandate-008 misread this exception forecloses. A finding that falsifies no criterion is FILED and not fixed in this task; trivia (cosmetic/style, language WARNs, pre-existing/unrelated/flaky) is non-blocking and recorded as a Flag.
...
6. **Adversarial verification probe**: fresh-context agent (or grep when sufficient) probes for false-pass scenarios. Probe verdict required before commit declared green. **Both the adversarial agent and the orchestrator's own probe can under-flag** — each constructs only the cases it thought of, so neither alone is sufficient: the orchestrator independently re-verifies the audit's load-bearing claims (don't relay a verdict), and the audit independently re-derives the orchestrator's. When either finds a defect that FALSIFIES an acceptance criterion, fixing it does NOT inherit the prior green — **the fix requires a FRESH re-audit against the falsified criteria**, because the fix can introduce its own defect or close only the reported instance of a class. A finding that falsifies no criterion is FILED, not fixed in this change. Loop fix→re-verify→re-audit until every criterion is CONFIRMED — even if total findings remain nonzero.
...
- **Append** (new item): write JSON to `/tmp/<id>.json`, then `append-block-item --block <name> --arrayKey <key> --autoId true --item @/tmp/<id>.json --writer … --json`. Use `read-schema --schemaName <name> --path properties.<key>.items.required` first when unfamiliar with the block's fields; `--arrayKey` is the block's `array_key` (`read-config --registry block_kinds --id <name>`). Birth edges (forcing-artifact / convention / decomposition links a new item must carry) go on the SAME call via `--relations '[{"relation_type":"…","direction":"as_parent|as_child","other":"<id>"}]'` (`direction` = the raw endpoint the NEW item occupies) — the item + its edges are one gate-judged atom; on an error-severity birth-edge invariant a bare filing is refused and the edge cannot be appended afterward. A role-bearing orientation-ambiguous relation_type (the gated-by / derived-from / supersedes / depends families) rejects the direction form — orient those entries with `"role":"primary|counter"` instead (the semantic role the NEW item holds, mapped via the relation's declared `role_direction`, exactly as `append-relation --primary/--counter`).
