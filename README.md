...
This process mode replaces the former `scripts/launch-constrained-pi.sh` launch script. `/context update`'s conflict-resolution path dispatches this same `pi-bound` surface as an interactive mergetool when a locally-modified schema has irreconcilable conflicts with the catalog.
