...
- `/context reconcile [--dryRun]` (CLI: `pi-context context-reconcile [--dryRun]`, auth-gated) — converge stored rollup-kind statuses with their derivation (the repair half of the `derived-status` invariant class): `--dryRun` previews the exact delta set; a live run applies it through the validated write path (identity-stamped, envelope-stamped, writer-attested). Never touches authored statuses or prose — authored statuses surface for review only through `context-validate`'s declared invariant signals; prose is not validated, its review is human
