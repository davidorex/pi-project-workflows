...
| **6** | 6.1 /project → /context subcommand rename • 6.2 project-* → context-* tool rename • 6.3 context-init (PROMPTS for substrate dir name AND block-kind selection — canonical_id/prefix/display_name per kind — so user never hand-authors config; basename/canonical_id binding realized by workflow construction) • 6.4 context-install (reads context-init-produced config + creates files matching declared canonical_ids) • 6.5 context-migrate • 6.6 internal-naming rename arc — `packages/pi-context/src/project-sdk.ts` → `context-sdk.ts`; `packages/pi-context/src/project-dir.ts` → `context-dir.ts`; `packages/pi-context/src/project-context.ts` → `context-context.ts` (or simpler `substrate-context.ts`); JS-export helpers `projectState` / `projectRoot` / `projectDir` / `projectTemplatesDir` → `contextState` / `contextRoot` / `contextDir` / `contextTemplatesDir`; `PROJECT_DIR` / `SCHEMAS_DIR` deprecated-const removal (already @deprecated); subpath exports in package.json updated; cross-package consumers cascaded; same scope discipline per `feedback_rename_scope_discipline.md` (scoped + bare-string scans; tsconfig exclude tmp + compiled-contexts) | 6.1/6.2 serialize per index.ts; 6.3/6.4/6.5 parallel-authorable; 6.6 large per-file rename with full bare-string cascade, serializes after 6.1/6.2 to avoid surface-level conflicts |
