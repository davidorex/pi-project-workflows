...
**Current position:** Cycles 1–9.3 committed; **Cycle 10: engine ✅ + `.project-migrate` canonicalized ✅ + active-substrate wiring ✅; `.context` fold-in + the rename remain.** `.project-migrate` is a frozen, structurally-canonical content-addressed archive of `.project`'s intent (588 items, 55 promoted entities + 55 membership edges, conventions registered; 0 nested-id / 0 dangling / 0 unregistered-relation_type; the remaining validateContext errors are legacy content-semantic, faithfully preserved + out of the structural canonicalizer's scope; now registered under alias `project` at `e4a1b95`). `.project` is pristine/untouched. The active substrate `.context-jit-spec-v2` is now **wired** (commits `071b393`→`b1bbfd0`): 70 items content-addressed; its 30 `project:FGAP-*` edges converted to structured foreign endpoints resolving into the registered archive (30/30 oid match); the `layer-plans` schema de-nested. **Founding FGAP-185 met** — `validateContext('.')`: `edge_endpoint_unregistered` **0**, `nested_id_bearing_array` **0**; the remaining **35 errors + 3 warnings** are pre-existing content-semantic legacy (target-kind mismatches, ungrounded decisions / orphan concept, verification edges), unchanged by the apply (before/after error-set identical modulo endpoint-label rendering). Adversarial audit PASS + orchestrator re-verification (`analysis/2026-06-01-cycle10-active-wiring-adversarial-audit.md`). **Remaining: `.context` fold-in** (light: mint/register substrate_id + land identity + backfill + convert its dangling `session_touches_item` edges + de-nest its empty layer-plans schema), **then the rename** (`.project`→`.project-archived`, `.project-migrate`→`.project` + registry dir update).
...
| 10 · `.context` fold-in | mint/register substrate_id + land identity on its schemas + backfill its items + convert its dangling `session_touches_item` edges + de-nest its empty `layer-plans` schema | ⏳ NEXT | — | — |
| 10 · rename (closing move) | `git mv .project .project-archived`; `git mv .project-migrate .project` + update registry dir (`sub-0c813fd8…` → `.project`) | ⏳ after fold-in | — | — |
