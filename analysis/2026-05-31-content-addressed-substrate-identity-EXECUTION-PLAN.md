...
| 3 | **C** | OID mint + `oid`/`content_hash`/`content_parent` on 16 schemas + `prepareItemIdentityForWrite` in ALL birth/mutation primitives + schema bumps + migration decls | First phase mutating existing write behavior across all primitives + all schemas | 0, A, B | live: append/upsert/update real items → all three fields stamped; rename ≠ hash change; oid immutable on update; legacy reads still validate |
