...
- **Defense-in-depth (three layers, all required, none substitutes for another):**
  1. **Structural floor:** the mandatory tier above — an override can never un-exclude `id`/`oid`/`content_hash`/`content_parent` regardless of authorization.
  2. **Action gate (already present):** authoring/editing any `x-identity` override is user-gated — the only path to declare it is a schema write through `write-schema`/`write-schema-migration`, both in `AUTH_REQUIRED_TOOLS` (`pi-agent-dispatch/src/auth-gate.ts:74-75`), so the write requires interactive operator confirm and is hard-refused non-interactively.
  3. **Informed authorization (BUILT IN CYCLE 3, required — not optional):** the gate is tool-name-level today and shows only a truncated arg summary, so a human can authorize an `x-identity` override without understanding it changes content-hash exclusions. Cycle 3 makes the confirm operation-aware: when a gated schema-write payload carries an `x-identity.metadata_fields` annotation, the confirm message surfaces exactly what the override changes — which discretionary fields it drops from (or adds to) the content-hash exclusion set — with an explicit affirmation that the mandatory floor stays excluded. So the operator authorizes an informed decision, not an opaque schema write. Mechanism: a pure `describeIdentityOverride(schema): string | null` helper in pi-context (the single source of the `x-identity` semantics) returns the human-readable delta vs the default, consumed by `authGateHandler` to enrich the confirm message; `null` (no `x-identity` present) leaves the existing confirm unchanged. Cross-package import direction (pi-agent-dispatch → pi-context) confirmed at implementation. Binding test: a `write-schema` payload with an `x-identity` override produces a confirm message naming the changed fields + the retained floor; a payload without one is byte-identical to today's confirm.
