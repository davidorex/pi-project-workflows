...
- Both files must be added to each package's `files` array in `package.json` IF not already present (verify; do NOT assume) so they ship to npm. **This `package.json` edit is the one place Part A touches package.json: the MANDATES bar this scoping artifact from modifying package.json, so the implementer dispatched from this plan performs it (D4), not this document.**
...
- **CI guard (required, D2):** add a step to `.github/workflows/ci.yml` running `scripts/check-changelog.mjs` against the PR's diff range so the rule holds even if a local husky hook is bypassed or absent. This is mandatory, not optional: husky is local-only and `--no-verify`-bypassable and external contributors' clones never run it, so CI is the only gate that keeps the published record truthful for outside contributions (D2).
...
- `.github/workflows/ci.yml` — CI enforcement of the changelog guard (required, D2).
...
- **D1 — backfill granularity: per-published-version, OSS-consumer scope.** The earlier consolidated-`[0.27.0]` recommendation rested on a falsified premise ("no per-version notes to reconstruct"); the evidence overturns it. (a) npm publish history is heterogeneous per package — pi-workflows 27 versions, pi-behavior-monitors 28, meta 26, pi-jit-agents 2, pi-context 1, pi-agent-dispatch 0 (re-derive `npm view "@davidorex/<pkg>" versions --json`); consumers received each published version, so each is owed an entry. (b) 51 tags `v0.3.0`…`v0.27.0` (re-derive `git tag | sort -V`) make a published version's range tag-bounded and path-grounded (A.3) — per-version is grounded, not fabricated. (c) `0.27.0` is tagged but unpublished (last npm = `0.26.0` everywhere), so backfill covers the public past only (≤ 0.26.0) and unpublished changes go to `[Unreleased]`. Granularity is per-published-version where many releases existed, consolidated where ≤ 2, fresh-start where never published (A.4).
- **D2 — CI enforcement: the changelog guard runs in CI alongside the husky commit-time guard (B.3).** Husky hooks are local-only and `--no-verify`-bypassable, and external contributors' clones don't run them at all; CI is the only gate that reliably keeps the *published* record truthful for an OSS project accepting outside contributions. The husky guard catches the solo-maintainer + agent path; CI catches everything else — so the CI guard is a required part of the forward discipline, not optional reinforcement (B.3, B.5).
- **D3 — sequencing: the backfill (Part A) lands before the commit-time guard switches on (Part B).** A guard switched on before backfill completes would block commits to packages whose backfill is mid-flight — the ordering A.7 and B already specify.
- **D4 — shipping the changelog: each package's `CHANGELOG.md` sits in its `package.json` `files[]` array.** A changelog absent from the published npm tarball is invisible to consumers who read the package via npm rather than the GitHub repo — `files[]` inclusion is what makes the backfill + forward-discipline effort consumer-visible. The Part-A implementer performs the edits (add to pi-agent-dispatch and pi-project-workflows; verify the four existing); this scoping artifact does not itself edit `package.json`.
- **D5 — a release fails when a package changed but recorded nothing.** When a package had published-surface commits since the last tag but its `[Unreleased]` is empty, the release hard-fails; a package with genuinely no published-surface change legitimately has an empty section and proceeds (informational line, no failure). A published release with a missing changelog entry is a broken promise to consumers; warn-and-proceed is how the dormancy persisted (B.3 R2b).

## What could not be determined
Per-version granularity is determinable: the published-version list (`npm view … versions`) crossed with the tags (`git tag`) bounds each public version's commit range, and path attribution (A.3) grounds each entry. The residual unknowns are narrower:
