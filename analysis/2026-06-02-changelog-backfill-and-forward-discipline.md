...
- `.github/workflows/ci.yml` — CI enforcement of the changelog guard (REQUIRED per resolved D2).
...
All of D1–D5 are settled. They are recorded here as decisions with their driving rationale, not as open scope calls.
...
- **D2 — CI enforcement. RESOLVED: YES — add the changelog guard to `.github/workflows/ci.yml` in addition to the husky commit-time guard (B.3).** OSS rationale: husky hooks are local-only and `--no-verify`-bypassable, and external contributors' clones don't run them at all; CI is the only gate that reliably keeps the *published* record truthful for an OSS project accepting outside contributions. The husky guard catches the solo-maintainer + agent path; CI catches everything else. Accordingly the CI guard is a REQUIRED part of the forward discipline, not optional reinforcement — B.3's CI line and B.5's CI codification point are updated to required.
- **D3 — Sequencing. RESOLVED: land the backfill (Part A) before switching on the commit-time guard (Part B).** Rationale: the commit-time guard, if switched on before backfill completes, would block commits to packages whose backfill is mid-flight. This is the ordering already specified in A.7 and B; it is settled as decided, not pending confirmation.
- **D4 — `package.json` `files` edits for the changelogs. RESOLVED: YES — each package's `CHANGELOG.md` MUST be in its `package.json` `files[]` array, and the Part-A implementer performs the needed edits (add to pi-agent-dispatch and pi-project-workflows; verify the four existing) as in-scope work.** OSS rationale: a changelog absent from the published npm tarball is invisible to consumers who read the package via npm rather than the GitHub repo — shipping it in `files[]` is what makes the whole backfill + forward-discipline effort consumer-visible. THIS scoping artifact does not itself edit `package.json` (the MANDATES bar it); the implementer dispatched from Part A does. The authorization is now settled, not pending.
- **D5 — Empty-`[Unreleased]`-with-changes at release. RESOLVED: hard-FAIL.** Fail the release when a package had published-surface commits since the last tag but recorded nothing under `[Unreleased]`. A package with genuinely no published-surface change since the last release legitimately has an empty section and proceeds (informational line, no failure). OSS rationale: a published release with a missing changelog entry is a broken promise to consumers; warn-and-proceed is exactly how the dormancy persisted. This confirms B.3 R2b as specified.
