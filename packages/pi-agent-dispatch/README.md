...
| `run-work-order-loop` | Single-call wrapper for the bounded north-star loop: dispatch target_agent (with the work-order's `context_blocks` additively merged onto the agent's own, and its return checked against `output_contract` when declared) → run-real-checks → on-pass commit-attested (routed through the same auth-gate confirm every `commit-attested` caller goes through) → on-fail human-OK retry, or the work-order's declared `on_fail` policy in a non-interactive context. Bounded iterations (default 3). |
...
A work-order may additionally declare `context_blocks` (additively merged onto the dispatched agent's own spec-declared `contextBlocks` before compile, so a work-order can hand off substrate context — e.g. `{ name: "work-orders", item: "WO-NNN" }` — without displacing what the agent's own spec already asks for) and `output_contract` (a JSON Schema the dispatched agent's return is checked against, right after dispatch and before the real-check runs; a violation throws naming the work-order id, mirroring `input_contract`'s guard). On a passing real-check, the commit no longer calls `attestedCommit` directly: it constructs the same `commit-attested` tool-call event every other caller of that tool produces and runs it through `authGateHandler` — an interactive context's confirm decision (allow/decline) governs the commit, and any verified-identity stamp the gate applies to the event's `writer` reaches the commit call; a non-interactive context skips the gate and the commit entirely, reporting `final_status: "completed-pending-commit"` with no `commit_sha`. A work-order may also declare `on_fail: "retry" | "abort"`: in a non-interactive context, `"retry"` proceeds to the next bounded iteration on a real-check failure instead of the default `aborted-non-interactive`; an interactive context always asks via `ctx.ui.confirm` regardless of `on_fail`.
