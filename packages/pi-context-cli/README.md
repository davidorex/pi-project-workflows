...
- **`append-block-item --dry-run`** (or `--dryRun`) previews the append and writes nothing: the op runs its live path — id-uniqueness, stamping, whole-file schema validation — on the exact prospective file, plus the birth-relations gate over any `--relations` entries (orientation, relation_type registration + endpoint-kind checks for resolvable endpoints, counter-endpoint resolution, prospective cycles including the new item's edges; the new item's own endpoint is exempt from existence checking). With `--autoId` it reports the id that would be allocated. The output is `would append item '<id>' to <block>.<arrayKey>` (plus `; would file N birth relation(s)` when relations are supplied); a refusal from these covered checks surfaces the live run's error text, while checks that need the written item or the post-write delta (e.g. the write-time invariant gate) do not run under preview. `upsert-block-item --dryRun` runs the same birth-relations gate for the append-mode upsert; its item-level preview covers stamping and whole-file schema validation, not the diff-scoped rhetorical-criteria check or the envelope schema_version stamp.
