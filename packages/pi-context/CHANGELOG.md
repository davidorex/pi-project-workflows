...
### Added
- `/context install --plan` previews installed-vs-catalog schema drift WITHOUT writing anything. The new read-only `planInstall(cwd): InstallPlan` (and its `renderInstallPlan(plan): string` summary) compares, per installed schema, the recorded install baseline (`config.installed_from.assets[name].content_hash`) against the catalog's current schema file and the currently-installed schema file, and classifies each as `in-sync` / `catalog-ahead` (the package shipped a newer schema) / `locally-modified` (the installed file was edited) / `both-diverged` / `no-baseline` / `missing-catalog` / `missing-installed`. Each per-asset entry also carries `baseline_version`, `catalog_version`, and `installed_modified`. Every file-hash read is wrapped so a corrupt file degrades to a `missing-*`/diverged classification rather than throwing. In the `install:` handler `--plan` takes precedence over `--update` and returns before any mutation path; the detector performs zero writes (no config write, no copy, no mkdir). A shared `resolveCatalog()` read helper (samples root + `canonical_id`→paths map) is extracted so the installer and the drift detector resolve the catalog identically.
