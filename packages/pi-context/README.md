...
**Ordering-edge direction (`endpoint_roles`).** The blocked/ready deriver reads an ordering edge as `{parent = prerequisite, child = dependent}` — the parent must complete before the child unblocks. Because `source_verb_target` relation names (e.g. `task_depends_on_task`) read the opposite (the verb implies the parent is the depender), an ordering relation_type can declare `endpoint_roles: { parent, child }` (each `"prerequisite"` or `"dependent"`) naming which role each AUTHORED endpoint holds. When present, `append-relation` / `append-relations` AUTO-ORIENT an authored edge to deriver-canonical storage: a relation_type declaring `{parent:"dependent", child:"prerequisite"}` has its authored parent/child swapped so the stored edge always lands prerequisite-at-parent (file the edge following the name verb; storage is corrected for you); `{parent:"prerequisite", child:"dependent"}` is stored verbatim. The orientation applies only to relation_types carrying `endpoint_roles` — ordering types without it, non-ordering types, and unregistered types are stored exactly as authored.
...
- `/context install [--update]` — reconcile the substrate against `installed_schemas` / `installed_blocks` in `config.json` by copying assets from the samples catalog (skip-if-exists by default). `--update` re-syncs installed schemas through the migration registry: a same-version body change is a verbatim re-sync (`resynced`); a `version` bump forward-migrates the populated block's items through the shipped migration chain and re-validates them against the new schema (`migrated`); a bump with no safe migration — no shipped chain, or items that would fail the new schema — is refused, leaving BOTH the schema and the block byte-unchanged (`blocked`). Populated block data is never overwritten (reported as `preserved`), and empty or absent blocks get the catalog starter. Install also records an **install baseline** in `config.installed_from` — the catalog source (`name@version` + conception `schema_version`) plus a per-schema fingerprint (content hash + declared version) of the installed schemas — used for installed-vs-catalog drift detection (re-synced/migrated schemas refresh their baseline so a follow-up `/context check-status` reports them `in-sync`). The baseline covers schemas only (blocks are user data); a re-install on an unchanged substrate is idempotent (byte-identical `config.json`, `at` preserved)
