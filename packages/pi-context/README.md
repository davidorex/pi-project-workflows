...
**Write-time rhetorical-criteria enforcement.** A schema's item subschema may carry `x-rhetorical-criteria` (an AJV-ignored extension keyword, sibling to `x-identity` / `x-prompt-budget` / `x-lifecycle`) with `downstream_consumer` (who consumes the block verbatim, and why), `register_notes` (the block-kind-specific terseness / self-containment / appropriateness guidance — human-review surface, unenforced), and `prohibited_patterns` (per-block `{pattern, applies_to, reason}` regex bans). The write path mechanically enforces two rules: per-field word caps read from `x-prompt-budget.words`, and a universal ban on provenance / git / prior-state narration (the exported `DEFAULT_PROHIBITED_PATTERNS`, e.g. `previously`, `no longer`, a bare commit SHA) applied to every budgeted string field, plus any schema-authored `prohibited_patterns`. A violation throws `RhetoricalValidationError` and refuses the write.

Enforcement covers a changed item at every level it actually appears at: a nested-array writer (`append-block-nested-item` / `update-block-nested-item`, e.g. `layer-plans`' `plans[].layers[]`) checks the item against its OWN nested item subschema, not the block's top-level one; and a whole top-level item written with nested-array content supplied inline (e.g. a `layer-plans` item created with `layers[]`/`migration_phases[]` already populated) has that inline content recursively checked too, each nested element against its own resolved subschema. Not yet reached: arrays of budgeted PRIMITIVE strings, where the budget sits on the array's own `items` schema rather than a named object property (e.g. `decisions.consequences[]`, `tasks`/`requirements.acceptance_criteria[]`) — `collectWordCaps` only reads `x-prompt-budget` off named properties, so these stay unenforced.
