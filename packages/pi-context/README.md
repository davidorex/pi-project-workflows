...
<substrate-dir>/
  ...
  agents/<name>.agent.yaml    — installed from samples/agents/ (editable project-tier specs; never overwritten)
  agents/schemas/<name>.schema.json — installed from samples/agents/schemas/ (a materialized spec's adjacent output schemas)
...
**Write-time rhetorical-criteria enforcement.** A schema's item subschema may carry `x-rhetorical-criteria` (an AJV-ignored extension keyword, sibling to `x-identity` / `x-prompt-budget` / `x-lifecycle`) with `downstream_consumer` (who consumes the block verbatim, and why), `register_notes` (the block-kind-specific terseness / self-containment / appropriateness guidance — human-review surface, unenforced), and `prohibited_patterns` (per-block `{pattern, applies_to, reason}` regex bans). The write path mechanically enforces two rules: word caps read from `x-prompt-budget.words` — declared on a named string property, or on a bare-string array's own `items` subschema (each string element capped individually) — and a universal ban on provenance / git / prior-state narration (the exported `DEFAULT_PROHIBITED_PATTERNS`, e.g. `previously`, `no longer`, a bare commit SHA) applied to every budgeted string field and every element of a budgeted bare-string array, plus any schema-authored `prohibited_patterns`. A violation throws `RhetoricalValidationError` and refuses the write.

Enforcement covers a changed item at every level it actually appears at: a nested-array writer (`append-block-nested-item` / `update-block-nested-item`, e.g. `layer-plans`' `plans[].layers[]`) checks the item against its OWN nested item subschema, not the block's top-level one; and a whole top-level item written with nested-array content supplied inline (e.g. a `layer-plans` item created with `layers[]`/`migration_phases[]` already populated) has that inline content recursively checked too, each nested element against its own resolved subschema. Arrays of budgeted PRIMITIVE strings — where the budget sits on the array's own `items` schema rather than a named object property (e.g. `decisions.consequences[]`, `tasks`/`requirements.acceptance_criteria[]`, `layer-plans`' `migration_phases[].exit_criteria[]`) — are enforced per element, at any nesting depth; a bare-string array whose items carry no budget is not checked.

Enforcement is diff-scoped — only the changed item(s) (and, per the above, their inline nested content) are checked, never the whole reconstructed file — so a pre-existing (grandfathered) violator on an untouched item never blocks an unrelated forward write, and whole-block overwrites (bulk import, migration reconciliation) opt out.
...
- **Block CRUD** — `read-block`, `write-block`, `read-block-dir`, `append-block-item`, `update-block-item`, `upsert-block-item` (validated find-or-append), `remove-block-item`, and the nested-array variants (`append/update/remove-block-nested-item`). `append-block-item` and append-mode `upsert-block-item` take an optional `relations` array filing the new item's birth edges in the same op run — item + edges judged as one atom by the write-time gate. Each entry names the relation_type, the other endpoint's selector, and exactly one orientation: `direction: as_parent|as_child` (raw — the endpoint the new item occupies) or `role: primary|counter` (role-typed — the semantic role the new item holds, mapped via the relation's declared `role_direction`; required for role-bearing orientation-ambiguous relation_types, where the raw form is rejected). `append-block-item --dryRun` and `upsert-block-item --dryRun` preview the filing without writing: the exact prospective file (stamped, whole-schema-validated) plus the birth-relations gate — orientation, relation_type registration + endpoint-kind checks for resolvable endpoints, counter-endpoint resolution (a dangling/unregistered counter-endpoint is refused with the live rejection text), and the prospective-cycle check — with the new item's own endpoint exempt from existence checking (unwritten by definition; it resolves once the live run appends the edges after the item write) while its edges still count in the cycle set. A preview refusal from these covered checks carries the live run's rejection text; checks that need the written item or the post-write delta (e.g. the write-time invariant gate) do not run under preview.
...
- **Lifecycle/state** — `context-status`, `context-validate`, `context-validate-relations`, `complete-task` (the closure atom: gates on a passing verification, files the `verification_verifies_item` edge itself — verification=parent, task=child; idempotent — then flips the task to completed in the same run; no prior `append-relation` step). The validator ops (`context-validate`, `context-validate-relations`, `context-roadmap-validate`) take optional narrowing parameters — `severity`/`code` filters on all three, a `block` filter on `context-validate` only (its issues carry a `block` field; the relation/roadmap validators declare no `block` axis and reject it as an unknown parameter), and `offset`/`limit` pagination over the returned `issues[]` — applied AFTER the full evaluation: the result's `status` always reflects the FULL evaluation (a filtered read never hides the true verdict), and any narrowed call carries a `slice` head (`totalIssues`/`matching`/`returned`/`offset`/`hasMore`). Unparameterized calls return the exact prior shape.
- **Substrate management** — `context-init`, `context-accept-all`, `context-install`, `context-switch`, `context-list`, `context-archive`.
- **Roadmap** — `context-roadmap-load`, `context-roadmap-render`, `context-roadmap-validate`: the derived roadmap over the `milestone_precedes_milestone` DAG — milestone-block items topo-ordered by the authored precedes edges, each milestone/phase carrying its authoritative completeness verdict in `status` and a separate per-milestone/per-phase `taskProgress` task-status aggregation (progress display ONLY — never a completeness verdict; read `status` for completeness); adjacency comes strictly from the edges, never inferred from order.

The relation byRef ops (`append-relation` / `remove-relation` / `replace-relation` / `append-relations`) and the filing ops (`append-block-item` / `upsert-block-item`) accept a `dryRun` flag: it resolves the operation and validates the prospective whole file under the SAME write-path validation, returning the would-decision while writing nothing. The same shared library path backs both the op (`--dryRun`) and the orchestrator scripts — one implementation, not a script-only preview.

**Op output.** Every op's `run` returns a structured `OpResult` (`string | { json } | { read }`) — data ops carry their un-stringified value, read ops a `ReadStructured`. The runtime-reflecting CLI's `--json` envelope emits `output` as a real JSON value (no double-encode); the default CLI text surface and the in-pi Pi-tool text surface route through `renderOpResultText`. The 50KB read cap is enforced at the OUTPUT BOUNDARY across all three channels (CLI `--json`, CLI text, in-pi text) via `boundedJsonOutput` / `renderOpResultText` — an over-cap payload fails closed (`--json` → `{ data: null, truncated: true, totalBytes, complete: false }`; text → a no-payload REFUSAL) rather than leaking unbounded substrate content. An op that exposes narrowing parameters declares an `overCapDirective` (mirroring the `{read}` channel's over-cap directive shape), and its boundary refusal names the concrete mechanism — each validator directs to its own axes (`severity`/`block`/`code` for `context-validate`; `severity`/`code` for `context-validate-relations` and `context-roadmap-validate`) or `offset`+`limit`, with the same sentence riding the fail-closed `--json` envelope under `directive` — so an over-cap result is always reachable through bounded slices; ops without one keep the mechanism-less refusal.
...
- `/context install` (CLI: `pi-context context-install [--update]`) — reconcile the substrate against `installed_schemas` / `installed_blocks` / `installed_agents` in `config.json` by copying assets from the samples catalog (skip-if-exists by default). Populated block data is never overwritten (reported as `preserved`), and empty or absent blocks get the catalog starter. Declared agent specs materialize under `agents/` (their output schemas under `agents/schemas/`) as editable project-tier specs and are never overwritten — not even under `--update` (an existing spec is reported `skipped`) — and never baselined; deleting one falls back to the bundled tier. Install also base-stamps each as-installed schema body into the object store and records an **install baseline** in `config.installed_from` — the catalog source (`name@version` + conception `schema_version`) plus a per-schema fingerprint (content hash + declared version) of the installed schemas — used for installed-vs-catalog drift detection. The baseline covers schemas only (blocks are user data, agents are editable project files); a re-install on an unchanged substrate is idempotent (byte-identical `config.json`, `at` preserved). On a substrate whose config carries no `substrate_id`, install establishes the identity at entry (mint + persist + register) and reports it under `substrateIdEstablished`. (Bringing the installed schema model current is now `/context update`, below.)
- `/context check-status` (CLI: `pi-context context-check-status`) — read-only: reports drift between the installed schemas and the catalog, reporting each as `in-sync` / `catalog-ahead` / `locally-modified` / `both-diverged` / `no-baseline` / `missing-catalog` / `missing-installed`, and for each schema behind the catalog (`catalog-ahead` / `both-diverged`) it surfaces `behind` and a `version_delta` — the baseline → catalog version pair (a declared version bump) or a content-only basis when the catalog body moved with the version string unchanged; writes nothing beyond the idempotent ceremony seeds of the catalog's `config` migration declarations and the catalog-implied block-schema chains into `migrations.json` (every substrate-lifecycle ceremony seeds before its first config read, so a version-lagging legacy substrate is diagnosable and a substrate missing a catalog-implied chain self-heals)
...
- `/context reconcile [--dryRun]` (CLI: `pi-context context-reconcile [--dryRun]`, auth-gated) — converge stored substrate state with its derivation, the repair half of the `derived-status` invariant class. For every block kind a `derived-status` invariant declares (paired with its `state_derivation.rollups` entry), computes each item's stored-vs-derived status delta using the SAME completeness helper the state derivation's gate satisfaction and `context-validate` use — preview, detector, and repair cannot disagree. The sweep also includes declared-baseline staleness: every `stale_conditions`-bearing item whose status buckets complete and whose typed condition fired (item-status / file-changed) transitions to `stale`, through the same `evaluateStalenessCandidates` verdict `context-validate` flags with. `--dryRun` returns the exact delta + transition sets a live run would apply (deltas: id, block, from/to, declaring invariant; `stalenessTransitions`: id, block, from, to stale, firing reasons), writing nothing; a live run applies exactly those sets through the standard validated write path (identity-stamped, envelope-stamped, writer-attested) and reports the applied counts. Scope: derived-status deltas + declared-staleness transitions ONLY — never an authored-status kind (feature/gap/issue/task buckets are human judgment), never prose; those classes are flagged for review by `context-validate`, not auto-repaired. A converged substrate is a clean no-op both ways; a live run on a pre-identity substrate establishes identity at entry
...
| `src/citation-rot-scanner.ts` | AST + JSON + markdown/YAML scanner enumerating canonical_id-shaped citations on shipped/operator-facing surfaces (`scanForCitationRot`); its default surfaces deliberately exclude comment trivia. A separate, additive `scanCommentsInFile` scans JSDoc/block/line comment trivia in a `.ts`/`.tsx` file's source text for the same `CITATION_RE` family (not wired into `scanForCitationRot`'s default walk) — for a caller doing its own before/after comment-citation comparison. `CITATION_RE`/`CITATION_RE_G` are exported. Exported subpath `./citation-rot-scanner`. |
