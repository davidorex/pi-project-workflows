...
- Block schema-validity sweep: every block with both an installed schema and a block file is validated whole against that schema (migration-aware when the envelope is version-stamped); each failure is an error (`block_schema_invalid`) naming the block, the failing item id where one is resolvable from the failure path (an envelope-level failure carries none), and the keyword — surfacing invalidity introduced without a write to the block (a hand edit, a schema resync, catalog drift).
