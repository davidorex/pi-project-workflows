...
| `dryRun` | boolean | no | Preview the append without writing — validates the exact prospective file and runs the full birth-relations gate (orientation + counter-endpoint resolution + prospective cycles; the new item's endpoint is exempt from existence checking) |
