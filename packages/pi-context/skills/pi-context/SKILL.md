...
Ordering-edge direction. The blocked/ready deriver reads an ordering edge as `{parent = prerequisite, child = dependent}` (parent must complete before child unblocks). An ordering relation_type may declare `endpoint_roles: { parent, child }` (each `"prerequisite"` or `"dependent"`) naming which role each AUTHORED endpoint holds — alongside `source_kinds`/`target_kinds`. When present, `append-relation` / `append-relations` auto-orient the authored edge to deriver-canonical storage: a relation_type whose name verb implies the parent is the depender declares `{parent:"dependent", child:"prerequisite"}` and has its authored parent/child swapped at write time so the stored edge always lands prerequisite-at-parent (file following the name; storage is corrected); `{parent:"prerequisite", child:"dependent"}` is stored verbatim. Orientation fires only for relation_types carrying `endpoint_roles`; types without it, non-ordering types, and unregistered types store exactly as authored.
