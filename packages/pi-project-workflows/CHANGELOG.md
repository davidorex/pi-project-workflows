...
- Regenerated the aggregate `SKILL.md` for the pi-context `write-schema-migration` op description now enumerating the `map_each` transform (table lookup with parent/child fallback endpoint; set-field-on-each-element) alongside rename/set/delete/coerce, and against the current generator output (the `<events>` section is no longer emitted); see the `pi-context` changelog for the member-package detail.
