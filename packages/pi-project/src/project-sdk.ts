...
// can import projectRoot without a cycle). Existing consumers importing
// from @davidorex/pi-project/project-sdk continue to resolve these names
// unchanged.
export {
	type ConfigBlock,
...
export function availableBlocks(cwd: string): BlockInfo[] {
	const workflowDir = path.join(cwd, projectRoot(cwd));
...
export function availableSchemas(cwd: string): string[] {
	const dir = path.join(cwd, projectRoot(cwd), SCHEMAS_DIR);
...
export function findAppendableBlocks(cwd: string): Array<{ block: string; arrayKey: string; schemaPath: string }> {
	const schemasDir = path.join(cwd, projectRoot(cwd), SCHEMAS_DIR);
...
export function schemaInfo(cwd: string, schemaName: string): SchemaInfo | null {
	const schemaPath = path.join(cwd, projectRoot(cwd), SCHEMAS_DIR, `${schemaName}.schema.json`);
	try {
...
export function blockStructure(cwd: string): BlockStructure[] {
	const blockDir = path.join(cwd, projectRoot(cwd));
...
export function projectState(cwd: string): ProjectState {
	...
	let phaseCurrent = 0;
	try {
		const phasesDir = path.join(cwd, projectRoot(cwd), "phases");
	...
	// Handoff presence
	try {
		const handoffPath = path.join(cwd, projectRoot(cwd), "handoff.json");
