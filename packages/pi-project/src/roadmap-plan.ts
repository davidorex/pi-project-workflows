...
import { type Edge, getProjectContext, type ItemRecord } from "./project-context.js";
...
export function loadRoadmap(cwd: string, roadmapId: string): RoadmapView | { error: string } {
	...
	const phases: PhaseView[] = roadmap.phases.map((phase) => {
		...
		const status = rollupPhaseStatus(items);
...
/**
 * Validate every roadmap × phase × milestone. Codes mapped to severity
 * downstream by mapRoadmapIssue in project-sdk.ts. Roadmap.json absent →
 * { status: "clean", issues: [] } (opt-in block; absence is not a defect).
 */
	...
	for (const roadmap of roadmaps) {
		...
		if (roadmap.status !== undefined && !ROADMAP_STATUS_ENUM.has(roadmap.status)) {
			issues.push({
				...
				message: `Roadmap '${roadmap.id}' has unknown status '${roadmap.status}'. Known: ${Array.from(ROADMAP_STATUS_ENUM).join(", ")}`,
		...
		for (const phase of roadmap.phases) {
			...
			if (!lensIds.has(phase.lens)) {
				issues.push({
					...
					message: `Phase '${phase.id}' in roadmap '${roadmap.id}' references unknown lens '${phase.lens}'.`,
					roadmap_id: roadmap.id,
					phase_id: phase.id,
				});
			} else {
				...
				if ("error" in lensView) {
					if (lensView.error.includes("composition_cycle_detected")) {
						issues.push({
							...
							message: `Phase '${phase.id}' in roadmap '${roadmap.id}' resolves to composition lens with cycle: ${lensView.error}`,
							roadmap_id: roadmap.id,
							phase_id: phase.id,
						});
					}
					...
					lensResolved = true;
					const status = rollupPhaseStatus(lensView.items);
					if (status.bucket === "unknown" && status.total > 0) {
						issues.push({
							code: "roadmap_status_unknown_value",
							message: `Phase '${phase.id}' in roadmap '${roadmap.id}' rolls up to bucket 'unknown' across ${status.total} item(s) — items lack a recognised status enum value.`,
							roadmap_id: roadmap.id,
							phase_id: phase.id,
						});
					}
				...
				if (milestone) {
					if (milestone.evidence_block && !blockNames.has(milestone.evidence_block)) {
						issues.push({
							...
							message: `Milestone '${milestone.id}' in roadmap '${roadmap.id}' references evidence_block '${milestone.evidence_block}' which is not loaded.`,
							roadmap_id: roadmap.id,
							phase_id: phase.id,
						});
					}
					if (milestone.evidence_query !== undefined) {
						...
						if (!ok) {
							issues.push({
								...
								message: `Milestone '${milestone.id}' in roadmap '${roadmap.id}' has invalid evidence_query — expected plain object with primitive values only.`,
								roadmap_id: roadmap.id,
								phase_id: phase.id,
							});
						}
		...
		for (const e of ctx.relations) {
			...
			if (parentIn !== childIn) {
				...
				issues.push({
					...
					message: `phase_depends_on edge {parent:${e.parent}, child:${e.child}} in roadmap '${roadmap.id}' references phase '${dangling}' that is not declared in this roadmap.`,
					roadmap_id: roadmap.id,
					phase_id: dangling,
				});
			}
		...
		for (const cycle of cycles) {
			issues.push({
				...
				message: `Phase cycle in roadmap '${roadmap.id}': ${cycle.join(" → ")}`,
				roadmap_id: roadmap.id,
				cycle,
			});
		}
