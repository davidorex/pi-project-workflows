 ...
 */
export async function checkForUpdates(notify: (message: string, level: "info" | "warning") => void): Promise<void> {
  ...
  if (isNewer(installed, latest)) {
    notify(
      `Update available: ${PACKAGE_NAME} ${installed} → ${latest}\nRun: pi install npm:${PACKAGE_NAME}`,
      "info",
    );
  }
}
