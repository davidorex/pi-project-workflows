...
All notable changes to `@davidorex/pi-workflows` are recorded here. Format follows [Keep a Changelog](https://keepachangelog.com/); entries are grounded in commits touching the published surface (`src/`, `skills/`, `workflows/`). Versions are per-published-npm-release; the last published version is `0.26.0`. Tagged-but-unpublished work (`0.27.0` and later) lives under `[Unreleased]`.

## [Unreleased]

- The bundled agent specs and their output schemas relocated to the pi-context samples catalog: `bundledDir("agents")` now resolves to pi-context's `samples/agents/` and `bundledDir("schemas")` to its spec-adjacent `samples/agents/schemas/`, both located via module resolution of the installed `@davidorex/pi-context` package (correct under the monorepo workspace symlink layout and under hoisted or nested published installs alike). `bundledDir("workflows")` still resolves this package's own `workflows/`. The package no longer ships `agents/` or `schemas/` directories; its `files` array drops both entries.
- `resolveSchemaPath` gains an existence-gated fallback for relative workflow step schema refs: a ref that does not resolve against the workflow file's own directory is probed against the bundled agents root before being returned. The bundled demo workflows now declare their step output schemas as `schemas/<x>.schema.json` (the same spec-adjacent form the agent specs use) and resolve to the catalog location from any install layout, with no cross-package relative path baked into the YAML. A ref that resolves file-relative behaves exactly as before; a ref that resolves at neither probe still returns the file-relative path so the downstream not-found error stays honest.
