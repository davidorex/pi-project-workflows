...
describe("extractToolArgsPreview", () => {
	...
	it("prefers command over other keys", () => {
		assert.strictEqual(extractToolArgsPreview({ command: "npm test", path: "/foo" }), "npm test");
	});
...
describe("dispatch", { skip: !hasPi ? "pi not available" : undefined }, () => {
	it(
		"runs a simple prompt and returns text output",
		async () => {
	...
	it(
		"returns failed status on bad prompt/agent",
		async () => {
			const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "wf-dispatch-"));
			const sessDir = path.join(tmpDir, "sessions");
			fs.mkdirSync(sessDir, { recursive: true });
	...
	it(
		"fires onEvent callback for process events",
		async () => {
			const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "wf-dispatch-"));
			const sessDir = path.join(tmpDir, "sessions");
			fs.mkdirSync(sessDir, { recursive: true });
...
describe("dispatch truncation", { skip: !hasPi ? "pi not available" : undefined }, () => {
	it(
		"sets truncated flag when stdout exceeds buffer limit",
		async () => {
			...
			const result = await dispatch({ agent: "default" }, { name: "default" }, "Respond with exactly: OK", {
				cwd: process.cwd(),
				sessionLogDir: sessDir,
				stepName: "test",
			});
