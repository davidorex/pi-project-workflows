...
import { ExpressionError, evaluateCondition, resolveExpression, resolveExpressions } from "./expression.js";
import type { CompletionScope, ExpressionScope } from "./types.js";
...
describe("resolveExpression", () => {
	...
	it("resolves nested step output", () => {
		assert.strictEqual(resolveExpression("steps.diagnose.output.fixLocation.file", scope), "src/auth/user-service.ts");
	});
...
describe("resolveExpression filters", () => {
	it("applies duration filter", () => {
		assert.strictEqual(resolveExpression("steps.diagnose.durationMs | duration", scope), "42s");
	});

	it("applies currency filter", () => {
		assert.strictEqual(resolveExpression("steps.diagnose.usage.cost | currency", scope), "$0.03");
	});
...
describe("resolveExpression with CompletionScope", () => {
	...
	it("resolves totalUsage fields", () => {
		assert.strictEqual(resolveExpression("totalUsage.cost", completionScope), 0.05);
	});

	it("applies filters on completion fields", () => {
		assert.strictEqual(resolveExpression("totalDurationMs | duration", completionScope), "1m32s");
		assert.strictEqual(resolveExpression("totalUsage.cost | currency", completionScope), "$0.05");
	});

	it("still resolves input and steps from completion scope", () => {
		assert.strictEqual(resolveExpression("input.path", completionScope), "/src");
		assert.strictEqual(resolveExpression("steps.diagnose.status", completionScope), "completed");
	});
});
...
describe("resolveExpressions", () => {
	...
	it("handles whitespace in expressions", () => {
		assert.strictEqual(resolveExpressions("${{   input.description   }}", scope), "null pointer in login");
	});

	it("renders undefined as empty string in embedded expressions", () => {
		const result = resolveExpressions("Question: ${{ input.nonexistent }}", scope);
	...
	it("stringifies objects in embedded expressions", () => {
		const result = resolveExpressions("Location: ${{ steps.diagnose.output.fixLocation }}", scope);
