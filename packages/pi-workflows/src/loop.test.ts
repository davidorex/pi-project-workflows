...
describe("loop steps", () => {
	it("breaks on gate pass", async (t: any) => {
		...
		t.after(() => {
			fs.rmSync(path.dirname(spec.filePath), { recursive: true, force: true });
			fs.rmSync(opts._cwd, { recursive: true, force: true });
		});
	...
	it("runs onExhausted when all attempts fail", {
		skip: !hasPi ? "pi not available" : undefined,
		timeout: 60000,
	}, async (t: any) => {
		const spec = makeSpec({
			steps: {
	...
	it("agent step inside loop with retry", {
		skip: !hasPi ? "pi not available" : undefined,
		timeout: 120000,
	}, async (t: any) => {
