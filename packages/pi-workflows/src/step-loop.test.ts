...
describe("executeLoop — direct unit tests", () => {
	...
	it(
		"breaks on first gate pass (onPass: break)",
		async (t) => {
	...
	it(
		"retries and exhausts when gate always fails (onFail: continue)",
		async (t) => {
			const loopSpec: LoopSpec = {
				maxAttempts: 2,
				steps: {
	...
	it(
		"stops on gate onFail: break without marking as completed",
		async (t) => {
			const loopSpec: LoopSpec = {
				maxAttempts: 3,
				steps: {
	...
	// Transform sub-steps
	it(
		"executes transform sub-steps within loop",
		async (t) => {
			const loopSpec: LoopSpec = {
				maxAttempts: 1,
				steps: {
	...
	it(
		"provides loop.iteration in scope for transforms",
		async (t) => {
			const loopSpec: LoopSpec = {
				maxAttempts: 2,
				steps: {
	...
	it(
		"provides loop.maxAttempts in scope",
		async (t) => {
			const loopSpec: LoopSpec = {
				maxAttempts: 5,
				steps: {
	...
	it(
		"provides loop.priorAttempts in scope",
		async (t) => {
	...
	it(
		"dispatches agent sub-steps via dispatchAgent callback",
		async (t) => {
	...
	it(
		"fails loop when agent sub-step fails",
		async (t) => {
			const loopSpec: LoopSpec = {
				maxAttempts: 3,
				steps: {
	...
	it(
		"includes agent sub-step usage in aggregated loop usage",
		async (t) => {
			const loopSpec: LoopSpec = {
				maxAttempts: 1,
				steps: {
	...
	it(
		"fails iteration when agent input expression resolution throws",
		async (t) => {
			const loopSpec: LoopSpec = {
				maxAttempts: 3,
				steps: {
	...
	it(
		"resolves maxAttempts from ${{ }} expression",
		async (t) => {
			const loopSpec: LoopSpec = {
	...
	it(
		"falls back to loopSpec.maxAttempts when expression resolves to NaN",
		async (t) => {
			const loopSpec: LoopSpec = {
				maxAttempts: 2,
	...
	it(
		"falls back to DEFAULT_MAX_ATTEMPTS when both are absent",
		async (t) => {
			const loopSpec: LoopSpec = {
	...
	it(
		"skips sub-step when its when condition is falsy",
		async (t) => {
			const loopSpec: LoopSpec = {
				maxAttempts: 1,
				steps: {
	...
	// onExhausted
	it(
		"runs onExhausted agent when all iterations fail without break",
		async (t) => {
	...
	it(
		"records expression error in onExhausted when input resolution fails",
		async (t) => {
			const loopSpec: LoopSpec = {
				maxAttempts: 1,
				steps: {
	...
	it(
		"does not run onExhausted when loop completed via break",
		async (t) => {
			let exhaustedDispatched = false;
			const loopSpec: LoopSpec = {
				maxAttempts: 3,
	...
	// Scope visibility
	it(
		"current iteration sub-steps are visible to later sub-steps in same iteration",
		async (t) => {
			const loopSpec: LoopSpec = {
				maxAttempts: 1,
				steps: {
	...
	it(
		"outer state.steps accessible from inside loop",
		async (t) => {
			const state = makeState({
				steps: {
					outer: {
	...
	// Cancellation
	it(
		"stops loop when signal is aborted",
		async (t) => {
	...
	// Output structure
	it(
		"result output has iterations, maxAttempts, attempts, lastIteration",
		async (t) => {
			const loopSpec: LoopSpec = {
				maxAttempts: 1,
				steps: {
	...
	it(
		"textOutput summarizes loop execution",
		async (t) => {
			const loopSpec: LoopSpec = {
				maxAttempts: 2,
				steps: {
