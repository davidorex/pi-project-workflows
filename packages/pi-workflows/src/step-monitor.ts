...
function loadPatterns(dir: string, patternsPath: string): MonitorPattern[] {
	try {
...
function loadInstructions(dir: string, instructionsPath?: string): MonitorInstruction[] {
...
function formatPatterns(patterns: MonitorPattern[]): string {
	return patterns.map((p, i) => `${i + 1}. [${p.severity ?? "warning"}] ${p.description}`).join("\n");
}

function formatInstructions(instructions: MonitorInstruction[]): string {
...
function createTemplateEnv(cwd: string): nunjucks.Environment {
	...
	try {
		...
		if (fs.existsSync(exDir)) searchPaths.push(exDir);
	} catch {
		/* not installed */
	}
...
function renderPrompt(
	...
	if (!spec.classify.prompt) return null;
	return spec.classify.prompt.replace(/\{(\w+)\}/g, (match, key: string) => {
...
function parseVerdict(raw: string): MonitorVerdict {
	const text = raw.trim();
