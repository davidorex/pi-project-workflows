...
describe("renderTemplateFile", () => {
	...
	it("supports {% extends %} inheritance", () => {
		...
		fs.writeFileSync(
			...
			['{% extends "base.md" %}', "{% block body %}Custom body for {{ focus }}{% endblock %}"].join("\n"),
		);
	...
	it("supports {% include %}", () => {
		...
		fs.writeFileSync(path.join(tmpDir, "main.md"), ["# Main", '{% include "fragment.md" %}', "End."].join("\n"));
	...
	it("supports {% macro %}", () => {
		...
		fs.writeFileSync(
			path.join(tmpDir, "macros.md"),
			["{% macro greeting(name) %}Hello, {{ name }}!{% endmacro %}"].join("\n"),
		);
		fs.writeFileSync(
			path.join(tmpDir, "main.md"),
			['{% from "macros.md" import greeting %}', "{{ greeting('Alice') }}", "{{ greeting('Bob') }}"].join("\n"),
		);
