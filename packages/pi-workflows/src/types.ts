...
export interface StepSpec {
	...
	/** @deprecated — reserved for nested workflow invocation. Not yet implemented. Parsed but rejected at validation time. */
	workflow?: string;
}
// Note: exactly one of agent, gate, transform, loop, parallel, monitor, command, or block must be set.
// workflow is reserved for nested workflow invocation (not yet implemented).
