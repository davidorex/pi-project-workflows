	...
	it("validates with all optional fields present", () => {
		const data = {
			...
			human_verification: [
				{
					name: "UI layout",
					test: "Open browser and check layout",
					expected: "Grid renders correctly",
					why_human: "Requires visual inspection",
				},
			],
			gaps: [{ truth: "Performance benchmark", status: "failed", reason: "No benchmark data available" }],
		};
		const result = validateFromFile(schemaPath, data, "test");
		assert.ok(result);
	});
...
describe("execution-results schema validation", () => {
	...
	it("validates a valid execution result", () => {
		const data = {
			...
			decisions: [{ id: "D-001", decision: "Use ESM", rationale: "Modern standard", status: "decided" }],
			issues: [{ severity: "info", description: "Minor lint warning" }],
			test_count: 42,
			commit_hash: "abc123",
		};
