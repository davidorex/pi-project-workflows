...
describe("discoverWorkflows", () => {
	it("discovers workflows from project directory", () => {
		...
		fs.writeFileSync(path.join(wfDir, "test.workflow.yaml"), "name: test\nsteps:\n  s:\n    agent: a");
	...
	it("skips invalid specs with warning (no throw)", () => {
		...
		fs.writeFileSync(path.join(wfDir, "good.workflow.yaml"), "name: good\nsteps:\n  s:\n    agent: a");
...
describe("findWorkflow", () => {
	it("finds a workflow by name", () => {
		...
		const wfDir = path.join(tmpDir, ".workflows");
		fs.mkdirSync(wfDir, { recursive: true });
		fs.writeFileSync(path.join(wfDir, "bugfix.workflow.yaml"), "name: bugfix\nsteps:\n  s:\n    agent: a");
