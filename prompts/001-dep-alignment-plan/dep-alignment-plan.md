...
<metadata>
  ...
  <open_questions>
    ...
    <question id="3">
      **tsconfig.json paths update**: The root tsconfig.json has `"@davidorex/pi-project/*": ["./packages/pi-project/*"]` which enables the current `@davidorex/pi-project/src/block-api.js` import path for type checking. After switching to named exports, these paths need updating. The glob path `"@davidorex/pi-project/*"` should map to the new subpath export targets. Confirm whether TypeScript project references would be preferable.
    </question>
    <question id="4">
      **pi-workflows missing pi-ai dependency**: `step-monitor.ts` imports `@mariozechner/pi-ai` (both type and value imports: `complete`, `StringEnum`) but pi-workflows' package.json does not declare it at all — not even as a peer dep. This is a pre-existing bug that should be fixed in this plan. Confirm scope inclusion.
    </question>
  </open_questions>
</metadata>
...
**Note**: The wildcard `@davidorex/pi-project/*` path could be preserved alongside named paths for backward compat during transition. However, removing it enforces the named-export contract at the TypeScript level too. Recommend removing it.
...
**Wait** — the wildcard export removal IS potentially breaking if any external consumer imports an unlisted subpath. The known consumers are pi-workflows (covered) and pi-behavior-monitors (will be covered). If there are no external consumers of unlisted subpaths, this is safe as minor. If there are, it should be major. User decision needed — see open question in metadata.
