...
- Write `compiled-contexts/neg-demo.ts` containing `import { writeBootstrapPointer } from "@davidorex/pi-context/context-dir"; writeBootstrapPointer("/tmp/x");`
...
- `npx tsx -e "import {writeBootstrapPointer, resolveContextDir} from '@davidorex/pi-context/context-dir'; import fs from 'node:fs'; import os from 'node:os'; import path from 'node:path'; const d = fs.mkdtempSync(path.join(os.tmpdir(),'fgap035-demo-')); writeBootstrapPointer(d, '.substrate-demo'); console.log(resolveContextDir(d));"`
...
(4) /context init UX demo — proves missing-arg path emits the declared ctx.ui.notify error:
- Demo inspection: `grep -n '/context init requires a substrate dir name' packages/pi-context/src/index.ts` returns 1 match (the new handleInit body's notify call)
